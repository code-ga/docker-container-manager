...
export const RoleForm: React.FC<RoleFormProps> = ({
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (isEdit && !canUpdateRoles) {
        console.error('Insufficient permissions to update role');
        return;
      }
      if (!isEdit && !canCreateRoles) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="flex items-center justify-end pt-4 space-x-3 border-t border-gray-200 dark:border-gray-700">
        ...
        <button
          type="submit"
          disabled={isLoading || (isEdit ? !canUpdateRoles : !canCreateRoles)}
