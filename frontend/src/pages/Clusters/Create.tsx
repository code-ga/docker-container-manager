...
import { useCreateCluster, useAddNodesToCluster } from '../../hooks/useClusters';
...
const CreateCluster = () => {
  ...
  const handleSubmit = async (data: Record<string, unknown>) => {
    if (!canCreate) {
      toast.error('You do not have permission to create clusters');
      return;
    }
    ...
    try {
      ...
      const cluster = await createClusterMutation.mutateAsync(clusterData);

      // If nodes are selected, add them to the cluster
      if (data.nodeIds) {
    ...
    <DashboardLayout>
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-400">
            Create a new cluster and assign nodes to it
          </p>
        </motion.div>
          ...
          >
            <div className="p-6 bg-gray-800/50 border border-gray-700 rounded-lg">
              <FormBuilder
                ...
                isLoading={createClusterMutation.isPending || addNodesMutation.isPending}
                className="space-y-6"
              />
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-gray-700">
                <Button
                  variant="secondary"
                  onClick={() => navigate('/clusters')}
                  disabled={createClusterMutation.isPending || addNodesMutation.isPending}
                >
                  Cancel
                </Button>
                <Button
