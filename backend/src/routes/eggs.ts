...
export const eggsRouter = new Elysia({ prefix: "/eggs" })
  .resolve(createPermissionResolve("egg:read"))
  ...
  // Write operations (POST, PUT, DELETE) - require egg:write permission
  .resolve(createPermissionResolve("egg:write"))
  .post(
    "/",
    async (ctx) => {
      const { name, image, startupCommand, envVars, config, description } =
        ctx.body;
    ...
    async (ctx) => {
      ...
      try {
        ...
        if (startupCommand !== undefined)
          updateData.startupCommand = startupCommand;
